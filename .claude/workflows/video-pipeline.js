...
- Bài mẫu chuẩn giọng: landing/video-remotion/videos/4-loai-thang-do-du-lieu/content.py.
...
const reviewOnce = (slug, round) => agent(
  `${CTX}\n\nSOÁT GIỌNG (vòng ${round}) file: landing/video-remotion/videos/${slug}/content.py
...
2. Nếu đã có landing/video-remotion/videos/${slug}/content.py: Read nó, chỉ TINH CHỈNH nếu cần (action "refined"/"kept"); nếu CHƯA có: viết mới (action "created").
...
4. Ghi file bằng Write vào landing/video-remotion/videos/${slug}/content.py (tạo thư mục nếu chưa có). Header comment ghi rõ nguồn blog + "chưa render, chưa gen audio".
...
1. Read landing/video-remotion/videos/${slug}/content.py để lấy beat mở làm hook.
...
4. Ghi bằng Write vào landing/video-remotion/videos/${slug}/thumb.json (JSON hợp lệ, không comment).
...
return {
  ...
  note: 'content.py + thumb.json đã ghi tại video-remotion/videos/<slug>/. CHƯA gen thumbnail PNG, CHƯA render (chờ user duyệt + nói "render").',
};
