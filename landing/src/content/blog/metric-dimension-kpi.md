...
Hãy hình dung dữ liệu của bạn như một **khối Rubik**. **Metric** là con số ghi trên mặt khối — thứ bạn muốn biết. **Dimension** là cách bạn xoay khối để nhìn con số đó từ một góc khác. **KPI** là vạch mục tiêu bạn vẽ lên tường, để biết con số trên khối đã chạm tới đó chưa.

Cùng một khối, cùng một con số — nhưng xoay khác đi thì thấy khác, và treo mục tiêu khác thì đánh giá khác. Ba việc tách bạch. Khi ba người trong phòng họp dùng lẫn lộn, họ tưởng đang cãi nhau về *con số*, trong khi thật ra mỗi người đang cầm một góc xoay khác nhau của cùng một khối.
...
- Tính theo ngày **đặt** hay ngày **giao thành công**?

Mỗi câu trả lời khác nhau cho ra một con số khác nhau — và cả năm đều "đúng" theo cách mỗi người hiểu. *Doanh thu của anh Sales* (đã chốt, gồm ship, chưa trừ hoàn) là 4,2 tỷ. *Doanh thu của chị Kế toán* (đã thu, trừ hoàn, không gồm ship) là 3,8 tỷ. Không phải sai số. Là **hai metric khác nhau đeo chung một cái tên**.
...
- Anh Sales và chị Kế toán cãi nhau vì **metric "doanh thu" chưa có một định nghĩa chung** — mỗi người một cách tính.
- Họ tưởng đang nói cùng một con số, nhưng thật ra mỗi người đang cầm một **dimension khác** (đã giao / đã thu, gồm ship / không ship) mà không nói rõ ra.
...
|---|---|---|
| **Metric** | Một đại lượng **đo được** bằng một con số | Doanh thu, số đơn, giá trị đơn trung bình |
| **Dimension** | **Lát cắt** để nhìn con số từ một góc | Theo kênh, theo tháng, theo sản phẩm |
| **KPI** | Một metric **gắn mục tiêu** để đánh giá tốt/chưa | "Doanh thu tháng đạt 4 tỷ — hiện 95%" |

Lần tới khi hai báo cáo lệch nhau, đừng vội đi tìm lỗi cộng trừ. Hỏi trước: *mình đang nói **metric** nào, cắt theo **dimension** nào, so với **KPI** nào?* Chín trên mười lần, "sai số" sẽ tan biến ngay khi ba từ này được gọi đúng tên. Đó là bước đầu tiên để mọi con số trong công ty ngừng cãi nhau.
