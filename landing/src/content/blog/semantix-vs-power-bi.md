...
Một chủ chuỗi bán lẻ ở TP.HCM kể với tôi: anh đã dùng Microsoft 365 cho cả công ty, nên khi được tư vấn "Power BI đi kèm sẵn, gần như miễn phí", anh gật đầu ngay. Cài Power BI Desktop, kéo vài bảng, làm được một biểu đồ cột. Rồi anh muốn một con số đơn giản: **tỷ lệ tăng trưởng doanh thu so với tháng trước, lọc theo từng chi nhánh.** Kéo thả không ra. Người tư vấn bảo "cái đó phải viết DAX". Ba tháng sau, dashboard vẫn trống — không phải vì Power BI yếu, mà vì **không ai trong công ty viết được DAX**, và thuê ngoài thì mỗi lần đổi câu hỏi lại phải gọi điện chờ.

Phản xạ đầu tiên khi nghe chuyện này là kết luận "Power BI dở". Sai. Power BI là công cụ BI **phổ biến nhất hành tinh** và xứng đáng với danh tiếng đó. Vấn đề không nằm ở công cụ — nó nằm ở chỗ Power BI được thiết kế cho một bài toán khác với bài toán của anh chủ chuỗi kia. Bài này nói thẳng về **Power BI cụ thể**: DAX, mô hình license, Desktop vs Service, gateway, Q&A tiếng Việt — những thứ đặc thù mà một bài so sánh chung chung sẽ bỏ qua.

> Nếu bạn muốn so sánh ở tầm cả nhóm BI dashboard truyền thống (Power BI, Tableau, Looker, Qlik) — triết lý dashboard-first vs question-first và bài toán chi phí per-user — tôi đã viết riêng trong [Semantix vs BI dashboard truyền thống](/blog/vs-powerbi-tableau/). Bài này thì khoan vào **một mình Power BI**.

## Vì sao đây không phải so "ai mạnh hơn"

Hãy nói sòng phẳng điều ít vendor nào chịu nói: Power BI **rất mạnh**. Kho biểu đồ phong phú, tích hợp sâu vào toàn bộ hệ Microsoft (Excel, Teams, SharePoint, Azure), cộng đồng hàng triệu người, và là tiêu chuẩn de facto của các tập đoàn lớn. Nếu bạn cần một dashboard tài chính 40 chỉ số với drill-down ba tầng, đẹp như tạp chí — Power BI làm điều đó tốt hơn gần như mọi thứ khác.
...
Đây là đặc thù số một của Power BI mà SME hay tính nhầm. Marketing nói "kéo thả không cần code", và điều đó đúng — *cho những phép tính đơn giản*. Tổng doanh thu, đếm số đơn: kéo thả ra ngay. Nhưng bất kỳ phép tính **nghiêm túc** nào — so sánh kỳ trước, tỷ lệ tăng trưởng, lũy kế từ đầu năm, tỷ trọng trên tổng — đều cần **DAX** (Data Analysis Expressions), ngôn ngữ công thức riêng của Power BI.

DAX trông không giống công thức Excel. Một measure tính tăng trưởng so với tháng trước có thể trông như thế này:
...
Một dashboard chỉ đáng tin khi số liệu **tươi**. Với Power BI, nếu dữ liệu của bạn nằm ở nguồn on-premise hoặc cần làm mới định kỳ, bạn phải dựng và **giữ chạy một on-premises data gateway** — một dịch vụ cầu nối để Power BI Service kéo dữ liệu về, cộng với lịch refresh (mấy lần một ngày, lúc nào).
...
Semantix sinh ra để hỏi bằng tiếng Việt như nói với một analyst giỏi. "Quý vừa rồi", "đầu năm đến giờ", "so với cùng kỳ năm ngoái" là input bình thường, không phải ca khó. Đây không phải khác biệt "có Q&A hay không có" — cả hai đều có — mà là Q&A được sinh ra cho ngôn ngữ nào.
...
Power BI có hàng trăm connector — nhưng phần lớn hướng tới hệ thống doanh nghiệp phương Tây (Dynamics, Salesforce, SQL Server, Azure). Để kéo Shopee và TikTok Shop về, thường phải qua **connector trung gian của bên thứ ba hoặc dựng pipeline thủ công** — lại cần người kỹ thuật, lại cần gateway giữ chạy. Tệ hơn: ghép ba nguồn này để ra **một** con số doanh thu hợp nhất là một bài toán dữ liệu thật, không phải kéo thả.
...
- Nhu cầu của bạn nghiêng về **visualization phức tạp**, dashboard tài chính nhiều tầng, báo cáo trình bày cầu kỳ pixel-perfect.
